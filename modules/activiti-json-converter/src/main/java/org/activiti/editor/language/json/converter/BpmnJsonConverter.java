...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
    ...
    public BpmnModel convertToBpmnModel(JsonNode modelNode) {
        ...
        if (nonEmptyPoolFound == false) {
            ...
            JsonNode processDocumentationNode =
                    modelNode.get(EDITOR_SHAPE_PROPERTIES).get(PROPERTY_DOCUMENTATION);
            if ((processDocumentationNode != null)
                    && StringUtils.isNotEmpty(processDocumentationNode.asText())) {
